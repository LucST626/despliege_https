...
async function fetchMessages() {
    try {
        const response = await fetch('http://dev5.cyberbunny.online:3000/', {
            method: 'GET',
            headers: {
                'APIKEY': API_KEY
            }
...
async function sendMessage(event) {
    ...
    try {
        const response = await fetch('http://dev5.cyberbunny.online:3000/?content=' + encodeURIComponent(content) + '&user=' + encodeURIComponent(user), {
            method: 'POST',
            headers: {
                'APIKEY': API_KEY
            }
